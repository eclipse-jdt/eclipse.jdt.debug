...
public class DeadLocksViewContentProvider implements ITreeContentProvider {
	...
	public Object[] getElements(Object inputElement) {
		...
		int numDeadLocks= MonitorManager.getDefault().getNumberOfDeadlocks();
		...
		for (int i = 0; i < numDeadLocks; i++) {
			...
			ContentThreadWrapper ctw = new ContentThreadWrapper();
			ctw.fThread = MonitorManager.getDefault().getStartThread(i);
			ctw.fParent = null;
			ctw.fDeadLockList =  MonitorManager.getDefault().getDeadlockList(i);
			ctw.caughtInADeadLock = true;
			roots[i] = ctw;
		}
		return roots;
