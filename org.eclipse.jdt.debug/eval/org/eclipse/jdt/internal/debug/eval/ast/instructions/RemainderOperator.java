...
public class RemainderOperator extends BinaryOperator {
	...
	protected int getIntResult(IJavaValue leftOperand, IJavaValue rightOperand) throws CoreException {
		...
		if (divisor == 0) {
			throw new CoreException(new Status(IStatus.ERROR, JDIDebugPlugin.getUniqueIdentifier(), IStatus.OK, InstructionsEvaluationMessages.getString("RemainderOperator.Divide_by_zero_1"), null)); //$NON-NLS-1$
		}
	...
	protected long getLongResult(IJavaValue leftOperand, IJavaValue rightOperand) throws CoreException {
		...
		if (divisor == 0) {
			throw new CoreException(new Status(IStatus.ERROR, JDIDebugPlugin.getUniqueIdentifier(), IStatus.OK, InstructionsEvaluationMessages.getString("RemainderOperator.Divide_by_zero_2"), null)); //$NON-NLS-1$
		}
